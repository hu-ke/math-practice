...
function hideSubGradeSection() {
    // 不清空currentSubGrade和currentSubSubGrade，保留用于提交答案时查找题目
    document.querySelector('.grade-section').style.display = 'block';
    document.getElementById('sub-grade-section').style.display = 'none';
    document.getElementById('sub-sub-grade-section').style.display = 'none';
...
function submitAnswers(grade) {
    ...
    // 获取题目key（支持多级子目录）
    let key;
    if ((grade === 1 || grade === 2 || grade === 3 || grade === 4 || grade === 5) && currentSubGrade === 'basic' && currentSubSubGrade) {
        // 基础练习使用二级子目录
        key = `${grade}-${currentSubGrade}-${currentSubSubGrade}`;
    } else if (currentSubGrade) {
        // 其他情况使用一级子目录
        key = `${grade}-${currentSubGrade}`;
    } else {
        key = grade;
    }
